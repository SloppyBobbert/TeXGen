...
<p align="center">
  ...
  <a href="#render-deployment">Render Deployment</a> •
...
The root [`render.yaml`](render.yaml) creates a Docker-based Django service, managed PostgreSQL database, and Vite static site. In the Render dashboard, create a Blueprint from this repository.

Before the first production deploy:

1. Choose globally unique service names if `texgen-api` or `texgen-web` are unavailable, then update the corresponding `https://<service>.onrender.com` values in `render.yaml`.
2. Set `YOUTUBE_API_KEY` on the API service if YouTube search is needed. It is intentionally not stored in the Blueprint.
3. For custom domains, add the API hostname to `DJANGO_ALLOWED_HOSTS`, set `CORS_ALLOWED_ORIGINS` to the frontend HTTPS origin, and set `VITE_API_URL` on the static site to the API HTTPS origin. Update the static site's CSP `connect-src` value in `render.yaml` if the API origin changes. Set `CSRF_TRUSTED_ORIGINS` only when using Django admin or cookie-based cross-origin workflows, then redeploy both services.

`DJANGO_SECRET_KEY` and `JWT_SIGNING_KEY` are generated by Render. The API container must provide its production web-server command through `backend/Dockerfile`; Compose overrides that command locally to wait for PostgreSQL, migrate, and run Django's development server.
