...
def test_canonical_create_emits_dual_contract_and_legacy_round_trips(auth_client):
    formula = FORMULAS[0]
    response = auth_client.post("/api/cheatsheets/", payload(formula["id"]), format="json")
...
@pytest.mark.django_db
def test_empty_selection_is_authoritative_and_dual_conflicts_are_rejected(auth_client):
    created = auth_client.post("/api/cheatsheets/", payload(), format="json").data
    response = auth_client.patch(f"/api/cheatsheets/{created['id']}/", {"revision": 1, "formula_selections": []}, format="json")
...
@pytest.mark.django_db
def test_template_id_is_canonical_alias_and_conflicts_with_template(auth_client):
    ...
    response = auth_client.post("/api/cheatsheets/", payload(template_id=first.id), format="json")
    assert response.status_code == 201
...
@pytest.mark.django_db
def test_unknown_or_duplicate_formula_ids_are_rejected(auth_client):
    response = auth_client.post("/api/cheatsheets/", payload("missing.nope"), format="json")
    assert response.status_code == 400
    formula_id = FORMULAS[0]["id"]
    response = auth_client.post("/api/cheatsheets/", payload(formula_selections=[{"formula_id": formula_id}, {"formula_id": formula_id}]), format="json")
    assert response.status_code == 400
...
def test_document_source_obeys_utf8_byte_limit(auth_client):
    assert auth_client.post("/api/cheatsheets/", payload(source_latex="éé"), format="json").status_code == 201
    assert auth_client.post("/api/cheatsheets/", payload(source_latex="ééé"), format="json").status_code == 400
